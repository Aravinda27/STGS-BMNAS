...
def main(args):
    ...
    print("common_suffix",common_suffix)
...
def get_duration(file):
    ...
    try:
        output = subprocess.run(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', file], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        ...
        print("Error during running command in get duration", error)
    # return round(float(output))  # ugly, but rounds your seconds up or down
    #print("Duration of file:",output)
...
def rescale_video(video_fn, w, h, fps, dir, new_dir, common_suffix, dict_video_length, ffmpeg, crf=17):
    ...
    # Create the dir
    #print("source video:",video_fn)
    video_dir_to_create = '/'.join(video_fn_rescaled.split('/')[:-1])
    #print("Video_dir:",video_dir_to_create)
    os.makedirs(video_dir_to_create, exist_ok=True)
    # Check if the file already exists
    #print("Video_fn_rescaled",video_fn_rescaled)
    ...
    # if True:
        try:
            subprocess.call(
                '{ffmpeg} -i {video_input}  -vf scale={w}:{h} -r {fps} -y {video_output} -loglevel panic'.format(
                    ffmpeg=ffmpeg,
            ...
            print("subprocess call in rescale video not executed!!!!")
        #print("Subprocess call executed!!!")
        # Get the duration of the new super_video (in sec)
        duration_sec = get_duration(video_fn_rescaled)
        #print("Duration in sec:", duration_sec)
...
def get_all_videos(dir, extension='.mp4'):
    ...
    #print("List_vidoe_fn:\n",list_video_fn)
    print("Total no of video_fn" , len(list_video_fn))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir', metavar='DIR',
                        default='/media/user1/8089c1c4-5bde-4b0d-abd1-ac7131e6ceb4/Aravind_data/codes/vinod/SWAN-DF/test/all_real_videos',
...
python prepare_ntu.py --common-suffix='rgb.avi'
