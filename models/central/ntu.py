...
import models.auxiliary.autospeech_master_resnet.resnet as sp_resnet
...
class Visual(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        x = x.contiguous()
        #print("This is input to resnet:",x.shape)
        #print("Resnet 34 of video Starts:")
        # Inflated ResNet
        ...
        #print("video fm5 shape:",out_5.shape)
        #print("vidoe fm6 shape:",out_6.shape)
        return out_1, out_2, out_3, out_4, out_5, out_6

class Spectrum(nn.Module):
    def __init__(self, args):
        super(Spectrum, self).__init__()
        self.cnn = sp_resnet.resnet34(2)
        # self.avgpool_1x7x7 = nn.AvgPool3d((1, 7, 7))
        self.avgpool_Tx7x7 = nn.AvgPool3d((8,8, 8))
        self.D = 512
        self.classifier = nn.Linear(self.D, args.num_outputs)
    ...
    def temporal_pooling(self, x):
        ...
        if self.D == D:
            final_representation = self.avgpool_Tx7x7(x)
            #print("final_representation shape: ", final_representation.shape)
            final_representation = final_representation.view(B, self.D)
            return final_representation
        else:
            print("Temporal pooling is not possible due to invalid channels dimensions:", self.D, D)
    ...
    def forward(self, x):
        ...
        #print(x.shape)
        # Inflated ResNet
        #print("Resnet for spectogram starts:")
        out_1, out_2, out_3, out_4 = self.cnn.featuremaps(x)
...
class CentralNet(nn.Module):
    ...
    def forward(self, input):
        ...
        self.skeleton.load_state_dict(torch.load("checkpoints/best_spectrogram"))
