...
def plot(genotype, filename, args, task=None):
    ...
    nw = Digraph(edge_attr=dict(fontsize='20', fontname="times", penwidth='1.5'),
            node_attr=dict(style='rounded, filled', shape='rect', align='center', 
                           fontsize='20', height='0.5', width='0.5', penwidth='2', 
                           fontname="helvetica"))
    nw.attr(rankdir='LR')
    nw.node("Video",fillcolor='lightgreen')
    ...
    with g.subgraph() as nothing:
        ...
        with c.subgraph(name='cluster_video_features', node_attr={'shape': 'box'}) as ca:
            ...
            input_nodes_A = ["Frame_1", "Frame_2", "Frame_3", "Frame_4"]
            if task == 'mmimdb':
            ...
            for input_node in input_nodes_A:
                ca.node(input_node, fillcolor='lightcoral')
        ...
        with c.subgraph(name='cluster_Spectogram_features', node_attr={'shape': 'box'}) as cb:
            cb.attr(style='rounded, filled', color='lightgrey', 
                           fontsize='20', align='center')

            input_nodes_B = ["Spectogram_1", "Spectogram_2", "Spectogram_3", "Spectogram_4"]
            if task == 'mmimdb':
            ...
            for input_node in input_nodes_B:
                cb.node(input_node, fillcolor='moccasin')
    ...
    # adding the nw subgraph edges
    nw.edge("Video", input_nodes_A[0], color='black')
    nw.edge("Video", input_nodes_B[0], color='black')
    g.subgraph(nw)
    ...
    for i in range(steps):
        ...
        node_x_name = "in1_C{}".format(i+1)
        node_y_name = "in2_C{}".format(i+1)
        node_z_name = "out_C{}".format(i+1)

        with g.subgraph(name=step_node_name, node_attr={'shape': 'box'}) as c:
            c.attr(style='rounded, filled', color='mediumpurple1', 
                   fontsize='20', align='center')
            ...
            #all_operation_cell_wise.append(operation_name)
            with c.subgraph() as ic:
                ...
                for inner_node_name in inner_node_names:
                    if inner_node_name != node_x_name and inner_node_name != node_y_name:
                        ic.node(inner_node_name, fillcolor='lightskyblue1')
            
            c.node(node_x_name, fillcolor='gold')
            c.node(node_y_name, fillcolor='deeppink1')
            c.node(node_z_name, fillcolor='lavender')
    ...
    g.node("Reduction\nOutput", fillcolor='slategray3')
    print("Node names:",node_names)
    for i in genotype.concat:
        g.edge(node_names[i], "Reduction\nOutput", color="blue")

    g.render(filename, view=True)
    #print("All operation cell wise:", all_operation_cell_wise)
